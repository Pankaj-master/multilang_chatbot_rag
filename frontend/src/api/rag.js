...
export default function Carousel({ items = [], columns = 3 }) {
  if (!Array.isArray(items) || items.length === 0) {
    return <div className="text-gray-500 p-4">No items to show.</div>
  }

  const colsClass = columns === 1 ? 'grid-cols-1' : columns === 2 ? 'sm:grid-cols-2' : 'sm:grid-cols-2 lg:grid-cols-3'

  return (
    <div className={`grid ${colsClass} gap-4`}>
      {items.map((it, idx) => (
        <ImageCard key={idx} item={it} />
      ))}
    </div>
  )
}
...
export default function ImageCard({ item }) {
  return (
    <div className="bg-white border rounded-lg p-3 shadow-sm flex flex-col">
      {item.image ? (
        <img src={item.image} alt={item.title || 'image'} className="w-full h-40 object-cover rounded-md" />
      ) : (
...
const CLIENT_BASE = (import.meta && import.meta.env && import.meta.env.VITE_API_BASE) || ''
const instance = axios.create({ baseURL: CLIENT_BASE })

export async function sendMessage(text, lang = 'en', opts = {}){
  if (!text) return { answer: '' }
  try{
    const payload = { query: text, lang, ...opts }
    const res = await instance.post('/chat', payload)
    // normalize response shape
    const data = res.data || {}
    return {
      answer: data.answer || data.text || (data.output && data.output.text) || '',
      cards: data.cards || data.items || data.cards || [],
      raw: data
    }
  }catch(err){
    ...
    return { answer: 'Error: ' + msg, cards: [], raw: {error: msg} }
  }
}
